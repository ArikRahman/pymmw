...
def xds110_reset_via_ccs():
    ccs_path = os.environ.get("CCS_PATH", r"C:\ti\ccs_base\common\uscif\xds110")
    exe = os.path.join(ccs_path, "xds110reset.exe")
    if os.path.exists(exe):
        try:
            result = subprocess.run([exe], capture_output=True)
            print("XDS110 NRST triggered")
        except Exception as e:
            print(f"Error running xds110reset: {e}")
    else:
        print("xds110reset.exe not found at", exe)
...
def _read_(
    prt, dat, timeout=2, handle=None
):  # observe control port and call handler when firmware is recognized
    script_path = os.path.dirname(os.path.realpath(sys.argv[0]))

    fw = [
        os.path.splitext(item)[0].split(os.sep)
        for item in glob.glob(os.sep.join((script_path, "mss", "*.py")))
    ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument(
            "-c",
            "--control-port",
            help="serial port for control communication",
            required=not nrst or "-n" in sys.argv or "--no-discovery" in sys.argv,
        )
        parser.add_argument(
            "-d",
            "--data-port",
            help="serial port auxiliary communication",
            required=not nrst or "-n" in sys.argv or "--no-discovery" in sys.argv,
        )
        parser.add_argument(
            "-f",
            "--force-handler",
            help="force handler for data processing (disables autodetection)",
            required=False,
        )
        parser.add_argument(
            "-n",
            "--no-discovery",
            help="no discovery for USB devices (avoids pre-access to the XDS debug probe)",
            action="store_true",
        )
        ...
        if nrst:
            try:
                ...
                if len(prts) != 2:
                    raise Exception("unknown device configuration detected")
        ...
        if args.control_port is None:
            args.control_port = prts[0]
        if args.data_port is None:
            args.data_port = prts[1]
        ...
        print_log(
            "control port: {} - data port: {}".format(args.control_port, args.data_port)
        )

        if args.force_handler:
            print_log("handler: {}".format(args.force_handler))

        tusr = threading.Thread(
            target=_read_,
            args=(con, args.data_port, None if not nrst else 2, args.force_handler),
        )
        tusr.start()
        ...
        if nrst:
            ...
            xds110_reset_via_ccs()  # <-- Automate XDS110 reset on Windows
            print(
