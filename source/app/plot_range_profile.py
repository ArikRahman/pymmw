...
try:
    ...
    while "lib" not in [
        d for d in os.listdir(__base__) if os.path.isdir(os.path.join(__base__, d))
    ]:
        __base__ = os.path.join(__base__, "..")
...
if __name__ == "__main__":
    if len(sys.argv[1:]) != 2:
        print(
            "Usage: {} {}".format(
                sys.argv[0].split(os.sep)[-1], "<range_maximum> <range_bias>"
            )
        )
        sys.exit(1)
